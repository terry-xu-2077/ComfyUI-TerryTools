...
function installStableCombo(widget, target) {
  ...
  try {
    Object.defineProperty(options, "values", {
      ...
      set() {
        // Other Terry extensions may refresh combo choices periodically. Ignore
        // those writes here: display labels are derived live from route names,
        // while the actual widget value remains an integer for prompt execution.
      },
    });
  } catch {}
...
app.registerExtension({
  ...
  setup() {
    // Low-frequency safety pass only for widgets created/rebuilt by another
    // extension (not for value rewriting). Once installed, accessors keep the
    // combo stable without periodic display-value mutation.
    setInterval(() => {
